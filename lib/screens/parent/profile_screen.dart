...
class ParentProfileScreen extends StatefulWidget {
  ...
  const ParentProfileScreen({Key? key, required this.user}) : super(key: key);
...
class _ParentProfileScreenState extends State<ParentProfileScreen> {
  ...
  Future<void> _loadProfile() async {
    ...
    try {
      final data = await ParentProfileService.getProfile();
      if (mounted) {
        setState(() {
  ...
  Future<void> _updateProfile() async {
    try {
      final result = await ParentProfileService.updateProfile(
  ...
  Future<void> _updateAvatar() async {
    ...
    if (image != null) {
      try {
        final result = await ParentProfileService.updateAvatar(image.path);
  ...
  Future<void> _changePassword() async {
    ...
    try {
      final result = await ParentProfileService.changePassword(
  ...
  Widget _buildContent() {
    final profile = _profileData!['profile'];
    final anakInfo = _profileData!['anak_info'] ?? [];
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Icon(Icons.family_restroom, color: Color(0xFF8B5CF6), size: 24),
                        SizedBox(width: 12),
                        Text(
                          'Anak Saya',
                          style: TextStyle(
                    ...
                    SizedBox(height: 12),
                    ...anakInfo.map<Widget>((anak) {
                      return Padding(
                        padding: EdgeInsets.symmetric(vertical: 4),
                        child: Row(
                          children: [
                            Icon(Icons.child_care, size: 16, color: Color(0xFF6B7280)),
                            SizedBox(width: 8),
                            Expanded(
                              child: Text(
                                anak['name'] ?? '',
